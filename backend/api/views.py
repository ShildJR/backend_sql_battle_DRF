...
from .serializers import (
    ...
    SubmitSolutionSerializer,
    AdminUserSerializer,
    AssignTaskSerializer,
    BulkAssignTasksSerializer,
    AssignTasksToGroupSerializer,
    UserGroupSerializer,
...
def submit_solution_view(request, task_id):
    ...
    if not validate_query(query):
        ...
        # Помечаем задачу завершенной даже при ошибке валидации
    ...
    if not success:
        ...
        # Помечаем задачу завершенной даже при ошибке выполнения
        assignment = TaskAssignment.objects.filter(
            user=user, task=task, completed_at__isnull=True
        ).first()
        if assignment:
...
def get_leaderboard_data():
    users = User.objects.filter(Q(total_points__gt=0) | Q(role="participant")).order_by(
        "-total_points"
    )[:50]
...
def _active_assignments_qs(user):
    ...
    now = timezone.now()
    return (
        TaskAssignment.objects.filter(user=user, completed_at__isnull=True)
...
def assigned_task_view(request):
    ...
    if not assignment:
        ...
        return Response(
            {"error": "Task not assigned"}, status=status.HTTP_404_NOT_FOUND
        )
...
def admin_users_view(request):
    ...
    return Response(
        AdminUserSerializer(users, many=True).data, status=status.HTTP_200_OK
    )
...
def admin_clear_assignment_view(request, user_id):
    ...
    return Response(
        {"success": True, "message": "Все назначения сняты"}, status=status.HTTP_200_OK
    )
...
def admin_tasks_view(request):
    ...
    if request.method == "GET":
        ...
        return Response(
            {"id": task.id, "title": task.title, "success": True},
            status=status.HTTP_201_CREATED,
        )

    return Response(
        AdminTaskSerializer(Task.objects.all(), many=True).data,
        status=status.HTTP_200_OK,
    )
...
def admin_groups_view(request):
    return Response(
        UserGroupSerializer(UserGroup.objects.all(), many=True).data,
        status=status.HTTP_200_OK,
    )
...
def admin_create_group_view(request):
    ...
    return Response(
        {"id": group.id, "name": group.name, "success": True},
        status=status.HTTP_201_CREATED,
    )
...
def admin_group_detail_view(request, group_id):
    ...
    group.delete()
    return Response(
        {"success": True, "message": "Группа удалена"}, status=status.HTTP_200_OK
    )
